...
def main():
    ...
    for ret, frame in video_iterable(video_capture):
        ...
        curr_time = time.time()
        if curr_time - start_time >= 0.2: 
            sqlite = "INSERT INTO people_counting (external_people, internal_people, door_status, timestamp) values(?, ?, ?, time('now','localtime'));"
            conn.execute(sqlite, (external_people, internal_people, 1 if elevator_doors_open else 0))
            conn.commit()
            start_time = curr_time
        # Write output video
        output_video_writer.write(frame)
        cv2.imshow("people_counting",frame)
        key = cv2.waitKey(1)
        if key == 27:
            break
